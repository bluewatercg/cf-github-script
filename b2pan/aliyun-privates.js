...
var AwsV4Signer = class {
  constructor({ method, url, headers, body, accesskeyID, secretAccessKey, sessionToken, service, region, cache, datetime, signQuery, appendSessionToken, allHeaders, singleEncode }) {
    ...
    this.region = region || guessedRegion || "cn-hongkong"; // 默认使用杭州区域
    
    // 阿里云OSS特殊处理
    if (this.service === "oss") {
      // 阿里云OSS使用V1签名
      this.useV1Signing = true;
      // 阿里云OSS要求使用GMT时间
      this.datetime = (/* @__PURE__ */ new Date()).toUTCString();
    } else {
      this.useV1Signing = false;
    ...
    // 阿里云OSS不需要X-Amz-Content-Sha256头
    if (this.service === "s3" && !this.signQuery && !this.headers.has("X-Amz-Content-Sha256") && !this.useV1Signing) {
      this.headers.set("X-Amz-Content-Sha256", "UNSIGNED-PAYLOAD");
    }
    
    if (this.useV1Signing) {
      // 阿里云OSS V1签名需要Date头
    ...
    if (this.service === "s3" || this.service === "oss") {
      try {
  ...
  async sign() {
    if (this.signQuery && !this.useV1Signing) {
  ...
  async authHeader() {
    if (this.useV1Signing) {
      // 阿里云OSS V1签名格式
      return "OSS " + this.accesskeyID + ":" + await this.signature();
    }
    return [
  ...
  async signature() {
    if (this.useV1Signing) {
      ...
      const stringToSign = [
        ...
        ...this.signableHeaders.map(h => h.startsWith("x-oss-") ? `${h}:${this.headers.get(h)}` : "").filter(Boolean),
        this.encodedPath
      ].join("\n");
      
      // 使用HMAC-SHA1进行签名
      const signature = await hmacSHA1(this.secretAccessKey, stringToSign);
      return signature;
    }
...
async function hmacSHA1(key, string) {
  const cryptoKey = await crypto.subtle.importKey(
    "raw",
    typeof key === "string" ? encoder.encode(key) : key,
  ...
  const signature = await crypto.subtle.sign("HMAC", cryptoKey, encoder.encode(string));
...
function guessServiceRegion(url, headers) {
  ...
  // 首先检查是否是阿里云OSS
  if (hostname.endsWith('.aliyuncs.com') || hostname.includes('.oss-')) {
    const regionMatch = hostname.match(/oss-([a-z0-9-]+)\.aliyuncs\.com/);
    const region = regionMatch ? regionMatch[1] : 'cn-hongkong'; // 默认杭州区域
    return ["oss", region];
  }
...
var my_proxy_default = {
  async fetch(request, env) {
    ...
    // 检查是否是阿里云OSS请求
    const isAliyunOSS = env.B2_ENDPOINT.includes('aliyuncs.com') || env.B2_ENDPOINT.includes('oss-');
    ...
    if (isAliyunOSS) {
      ...
      client = new AwsClient({
        ...
        "service": "oss",
        "region": env.B2_REGION || 'cn-hongkong' // 默认杭州区域
      });
    } else {
      // 原有Backblaze客户端配置
    ...
    // 对于阿里云OSS，需要特殊处理Range头
    if (isAliyunOSS && signedRequest.headers.has("range")) {
